...
const int NUMEFFECTSCHANNELS = 3;
...
bool CSynthesizer::Generate(double * frame)
{
    ...
    while (m_currentNote < (int)m_notes.size())
    {
        ...
        if (note->Instrument() == L"FlangingEffect")
        {
            m_flanging.SetSampleRate(GetSampleRate());
            m_flanging.SetNote(note);
            m_flanging.Start();
            m_send[1] = 1;
            m_send[0] = 0;
        }
        if (note->Instrument() == L"ChorusEffect")
        {
        ...
        if (note->Instrument() == L"ReverbEffect")
        {
    ...
    //
    double channelframes[NUMEFFECTSCHANNELS + 1][2];
    for (int i = 0; i < NUMEFFECTSCHANNELS + 1; i++)
    {
    ...
    for(list<CInstrument *>::iterator node = m_instruments.begin();  node!=m_instruments.end();  )
    {
        ...
        if(instrument->Generate())
        {
            ...
            for (int i = 0; i < NUMEFFECTSCHANNELS + 1; i++) {
                for (int c = 0; c < GetNumChannels(); c++) {
                    channelframes[i][c] += instrument->Frame(c) * m_send[i];
                }
            }
        }
        else
   ...
   //
    if (m_send[0] != 0) {
        ...
        double fframe[2] = { 0, 0 };
        if (m_send[1] != 0) {
            m_flanging.Process(channelframes[1], fframe);
            effect_num++;
        }
        double cframe[2] = { 0, 0 };
        if (m_send[2] != 0) {
            m_chorus.Process(channelframes[2], cframe);
            effect_num++;
        }
        double rframe[2] = { 0, 0 };
        if (m_send[3] != 0) {
            m_reverb.Process(channelframes[3], rframe);
            effect_num++;
        }
        for (int i = 0; i < 2; i++)
        {
            frame[i] = (rframe[i] + cframe[i] + fframe[i])/effect_num;
        }
    }
    //

...
CFlangingEffect::CFlangingEffect(void)
{
}
...
void CFlangingEffect::Process(double* frameIn, double* frameOut)
{
	...
	double delay = m_delay * sin(2 * PI * m_freq * m_time);
	int delaylength = int(delay * GetSampleRate() + 0.5);
	int rdloc = (m_wrloc + QUEUESIZE - delaylength) % QUEUESIZE;

	frameOut[0] = frameIn[0] / 2 + m_inQueueL[rdloc] / 2;
	frameOut[1] = frameIn[1] / 2 + m_inQueueR[rdloc] / 2;
...
void CFlangingEffect::SetNote(CNote* note)
{
